...
-- 🔧 FUNCTION TO ATTACH ALL CALLBACKS TO ENTITY
local function attachEntityCallbacks(entity, entityIndex)
...
-- Attach callbacks to initial entity
attachEntityCallbacks(listOfEntities[1], 1)
...
Communicator:Listen("SpawnEntity", function(sender: Player, id: number)
	...
	local entity = listOfEntities[id]
	if entity and entity.Model and entity.Model.Parent then
		print("⚠️ Entity already active!")
		return
	end

	-- Recreate entity if it was destroyed
	if not entity or not entity.Model or not entity.Model.Parent then
		listOfEntities[id] = Spawner:Create({
			Entity = {
				Name = "A60",
				Asset = "https://github.com/ppG64tre-Cool/Entity-spawn/raw/main/ReModelA60HC.rbxm",
				HeightOffset = 1
		...
		-- Reattach callbacks to new entity
		attachEntityCallbacks(listOfEntities[id], id)
		print("✅ Entity recreated!")
	end
